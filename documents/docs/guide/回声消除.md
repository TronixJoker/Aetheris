...
Aetheris adopts a cross-platform adaptive strategy, selecting the optimal solution based on each operating system's characteristics:
...
```text
┌─────────────┐    ┌─────────────────┐    ┌──────────────┐
│ Physical Mic│───▶│  WebRTC AEC     │───▶│ Aetheris   │
...
Aetheris automatically executes the following at startup:
...
```python
# Check AEC status after launching Aetheris
from src.audio_codecs.audio_codec import AudioCodec
